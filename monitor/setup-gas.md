  ...
  try{
    ...
    if(data.type === 'event'){
      ...
      writeRow_(ss, SHEET_SUB,
        ['申込日時','状況','役職','店舗名','流入元','店舗コード','お名前','電話','メール','希望連絡','面談希望','面談メモ','同意','自由記入'],
        [nowJST_(), data.status||'', data.role||'', data.store_name||'', data.source||'', data.store||'',
         data.name||'', data.tel||'', data.email||'', data.prefer||'', data.time||'', data.time_note||'', data.consent||'', data.note||'']);
    }
...
**「計測」シート**（イベントごと）
| 日時 | イベント | 流入元 | 店舗コード | 役職 | 補足 |
- `form_start`（フォーム開始）／`copy_intro`（紹介文コピー）／`share_owner_url`（オーナーURL共有）／`meeting_apply`（面談申込）／`cta_click`（ボタン押下）など
